...
| 硬件信息 | 推荐版本 & 模式 | 说明 |
| --- | --- | --- |
| **千兆网 + 机械盘 (HDD) + 任意内存**（例如 OVH KS-2 16T HDD） | 4.x + `-m 2` | **适用于机械盘的稳定组合。** 利用 4.x 的纯内存缓存和系统 Page Cache，将随机读写在内存中合并后写入磁盘，可有效避免 I/O 阻塞。 |
| **2.5G/万兆网 + 固态盘 (SSD) + 8G~16G 内存**（例如 Netcup G9.5） | 5.x + `-m 1` | **新版脚本的优势场景。** SSD 能应对随机读写，配合 5.x 的 Direct I/O 直接读写硬盘，效率更高。最新脚本引入动态自适应参数，`-m 1` 模式可充分利用带宽，同时避免内存溢出。 |
| **万兆网 (10G) + NVMe 固态 + ≥32G 内存**（例如 OVH 10G 高配独服） | 5.x + `-m 1` | **高性能配置。** 万兆网络下，脚本会自动分配大量连接和大 TCP 缓冲区，充分发挥硬件性能。 |
...
```bash
wget -qO asp.sh https://raw.githubusercontent.com/yimouleng/Auto-Seedbox-PT/main/auto_seedbox_pt.sh && bash asp.sh -u 用户名 -p 密码 -q 5 -m 1 -c 2048 -v -f -t -o
```
